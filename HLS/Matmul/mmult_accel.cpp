 ...
 */
#define MAX_N 768    // Maximum number of rows in matrix A and output C
 ...
 * and matrix A in blocks of BLOCK_M rows.
 ...
 * int64_t for outputs provides sufficient dynamic range for accumulated results.
 */
#define DTYPE_IN int8_t
#define DTYPE_OUT int64_t
...
extern "C"
{
     ...
     * 
     * This function implements an optimized matrix multiplication (C = A×B)
     * 
                     ...
                     int N, int K, int M, int update_A)
    {
        ...
        // #pragma HLS INTERFACE m_axi port = A offset = slave bundle = gmem depth = MAX_N * MAX_K
        // #pragma HLS INTERFACE m_axi port = B offset = slave bundle = gmem depth = MAX_K * MAX_M
        // #pragma HLS INTERFACE m_axi port = C offset = slave bundle = gmem depth = MAX_N * MAX_M

        // #pragma HLS INTERFACE m_axi port = A offset = slave bundle = gmem depth = 4096
        // #pragma HLS INTERFACE m_axi port = B offset = slave bundle = gmem depth = 4096
        // #pragma HLS INTERFACE m_axi port = C offset = slave bundle = gmem depth = 4096
        ...
        #pragma HLS INTERFACE m_axi port=A offset=slave bundle=gmem depth= MAX_N * MAX_K  max_read_burst_length=16 max_write_burst_length=16 data_width=32
        #pragma HLS INTERFACE m_axi port=B offset=slave bundle=gmem depth= MAX_K * MAX_M  max_read_burst_length=16 max_write_burst_length=16 data_width=32
        #pragma HLS INTERFACE m_axi port=C offset=slave bundle=gmem depth= MAX_N * MAX_M  max_read_burst_length=16 max_write_burst_length=16 data_width=32
        ...
        //********************************************************************
        // Persistent On-Chip Storage for Matrix A: --> too much BRAM util
        ...
        //********************************************************************
        // Block-based Processing of Matrix A:
        // Process matrix A in column blocks to optimize memory usage
        // Each block is loaded into on-chip BRAM for efficient access
        //********************************************************************

        outer_i_block:
        for (int i_block_A = 0; i_block_A < N; i_block_A += BLOCK_M){
            int current_block_M_A = ((i_block_A + BLOCK_M) <= N) ? BLOCK_M : (N - i_block_A);

            // Define BRAM for matrix A to load row-wise (BLOCK_M rows, MAX_K columns)
            DTYPE_IN A_bram[BLOCK_M][MAX_K];
            #pragma HLS BIND_STORAGE variable=A_bram type=ram_2p impl=bram

            // The block of rows in A corresponding to the current block in B
            int start_row_A = i_block_A;  // The starting row index for A

            // Load the current tile of matrix A (row-wise)
            copy_A_tile:
            for (int i = 0; i < current_block_M_A; i++) {  // Looping over the rows
                for (int k = 0; k < K; k++) {  // Looping over the columns
                    #pragma HLS PIPELINE II=1
                    A_bram[i][k] = A[(start_row_A + i) * K + k];  // Load A row by row
                }
            }
            ...
            for (int j_block = 0; j_block < M; j_block += BLOCK_M) {
                ...
                //****************************************************************
                tile_i:
                for (int i0 = 0; i0 < current_block_M_A; i0 += TILE_SIZE) {
                    tile_j:
                    for (int j0 = 0; j0 < current_block_M; j0 += TILE_SIZE) {
                        ...
                        DTYPE_OUT localA[TILE_SIZE][TILE_SIZE];
                        DTYPE_OUT localB[TILE_SIZE][TILE_SIZE];
                        #pragma HLS ARRAY_PARTITION variable=localA dim=0 complete
                        #pragma HLS ARRAY_PARTITION variable=localB dim=0 complete

                        // Initialize A and B to 4x4 matrices filled with 0s -> implied padding to make 
                        // 4x4 matrices for strassen
                        ...
                        for (int k0 = 0; k0 < K; k0 += TILE_SIZE) {
                            ...
                            for (int ii = 0; ii < TILE_SIZE; ii++) {
                                for (int kk = 0; kk < TILE_SIZE; kk++) {
                                    ...
                                    int global_i = i0 + ii;
                                    int global_k = k0 + kk;
                                    if (global_i < current_block_M_A && global_k < K)  
                                        localA[ii][kk] = static_cast<int64_t>(A_bram[global_i][global_k]);
                                    else
                                        localA[ii][kk] = 0;
                                }
                            }
                            ...
                            for (int kk = 0; kk < TILE_SIZE; kk++) {
                                for (int jj = 0; jj < TILE_SIZE; jj++) {
                                    ...
                                    if (global_k < K && global_j < current_block_M)
                                        localB[kk][jj] = static_cast<int64_t>(B_bram[global_k][global_j]);
                                    else
                                        localB[kk][jj] = 0;
                                }
                            }
                        ...
                        for (int ii = 0; ii < TILE_SIZE; ii++) {
                            for (int jj = 0; jj < TILE_SIZE; jj++) {
                                ...
                                if (global_i < N && global_j < current_block_M)
                                    C[(i_block_A + global_i) * M + (j_block + global_j)] += localC[ii][jj];
                            }
                        }
                    }
                }
