// THE update_A feature is not used here since we implemented tiling for A //
 ...
 */
static void reference_mmult(const int8_t *A, const int8_t *B, int64_t *C,
                            int N, int K, int M)
{
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            int64_t sum = 0; // Use 64-bit accumulation to avoid intermediate overflow
            for (int k = 0; k < K; k++) {
                sum += (int64_t)A[i * K + k] * (int64_t)B[k * M + j];
            }
            C[i * M + j] = (int64_t)sum;
        }
    }
...
int main()
{
    ...
    for (int t = 0; t < num_tests; t++) {
        ...
        int64_t *C_hw = new int64_t[maxC];
        int64_t *C_sw = new int64_t[maxC];
        ...
        for (int i = 0; i < N * M; i++) {
            if (C_hw[i] != C_sw[i]) {
                ...
                std::cout << "   Q projection mismatch at index " << i 
                          << ": HW = " << C_hw[i] 
                          << ", SW = " << C_sw[i] << std::endl << std::flush;
                found_mismatch = true;
                break;
            }
        }

        std::cout << "[DEBUG] test_pass = " << test_pass << "\n";

        if (test_pass)
            std::cout << "   Q projection Passed." << " " << C_hw[10] << " : " << C_sw[10] << std::endl << std::flush;
        else {
            std::cout << "   Q projection Failed at : " << mismatch_idx << " " << C_hw[0] << " : " << C_sw[0] << std::endl<< std::flush;
            overall_pass = false;
        }

        if (!found_mismatch) {
            std::cout << "No mismatches found in validation loop." << std::endl;
        }
